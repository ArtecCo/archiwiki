...
const isWikiCaptureRegex = (regex) =>
  regex instanceof RegExp &&
  regex.source === WIKI_CAPTURE_RE.source;

const isWikiPlaceholderRegex = (regex) =>
  regex instanceof RegExp &&
  regex.source === WIKI_PLACEHOLDER_RE.source;
  ...
  String.prototype.replace = function (searchValue, replaceValue) {
    if (isWikiCaptureRegex(searchValue) && typeof replaceValue === "function") {
      ...
      return originalReplace.call(this, searchValue, function (match, target, label, offset, source) {
        const index = capturedWikiLinks.length;
        capturedWikiLinks.push({
    ...
    if (isWikiPlaceholderRegex(searchValue) && typeof replaceValue === "function") {
      return originalReplace.call(this, searchValue, function (match, index, offset, source) {
        ...
        // Existing parser intentionally sees this as a missing note. Replace
        // only that result with a clickable header-aware wiki link.
        return `\n          <span class="wiki-link archiwiki-header-wiki-link underline cursor-pointer font-semibold" data-archiwiki-note-title="${escapeAttr(noteTitle)}" data-archiwiki-header="${escapeAttr(header)}" title="Open ${escapeAttr(noteTitle)} → ${escapeAttr(header)}">\n            ${escapeText(item.label)}\n          </span>\n        `;
      });
    }
...
const findSidebarNote = (title) => {
  ...
  return Array.from(document.querySelectorAll(".archiwiki-sidebar-item"))
    .find((item) => {
      const span = item.querySelector("span.truncate");
      return span?.textContent?.trim().toLowerCase() === wanted &&
...
const findHeading = (root, target) => {
  ...
  return Array.from(root.querySelectorAll("h1, h2, h3, h4, h5, h6"))
    .find((heading) => slugify(heading.id || heading.textContent) === wanted);
};
...
const buildTree = (container, root) => {
  ...
  headings.forEach(({ heading, title, level }) => {
    const row = document.createElement("button");
    row.type = "button";
    row.className = "block w-full text-left py-1 px-2 rounded hover:bg-neutral-200/70 transition-colors truncate";
    row.style.paddingLeft = `${Math.max(0, level - 2) * 12 + 8}px`;
...
const enhanceBacklinksPanel = () => {
  const panel = Array.from(document.querySelectorAll("#root .border-l"))
    .find((element) => element.textContent.includes("Backlinks ("));
  ...
  backlinksButton.type = structureButton.type = "button";
  backlinksButton.textContent = "Backlinks";
  structureButton.textContent = "Structure";
  backlinksButton.className = "px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider bg-neutral-200/70";
  structureButton.className = "px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider text-neutral-400";
  structure.className = "hidden flex-1 overflow-y-auto";

  tabs.append(backlinksButton, structureButton);
  ...
  structureButton.addEventListener("click", () => {
    structureButton.className = "px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider bg-neutral-200/70";
    backlinksButton.className = "px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider text-neutral-400";
