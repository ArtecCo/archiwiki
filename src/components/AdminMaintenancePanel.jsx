...
export default function AdminMaintenancePanel() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (mounted) setAuthorized(snap.exists() && snap.data()?.active === true);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      maintenanceRef,
      (snap) => setEnabled(snap.exists() && snap.data()?.inMaintenance === true),
      (err) => {
  ...
  const toggle = async () => {
    ...
    try {
      await setDoc(
        maintenanceRef,
        { inMaintenance: !enabled, updatedAt: Date.now() },
        { merge: true }
      );
    } catch (err) {
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-neutral-500">
            {enabled ? "Users are currently blocked." : "Users can access ArchiWiki."}
          </p>
        </div>
        <button
          type="button"
          ...
          className={"relative h-7 w-12 shrink-0 rounded-full transition-colors disabled:opacity-50 " + (enabled ? "bg-neutral-900" : "bg-neutral-300")}
        >
