...
export default function MaintenanceGate({children}) {
  ...
  return <div className={`min-h-screen flex items-center justify-center p-6 ${palette.page}`}><div className={`w-full max-w-lg rounded-xl border p-8 sm:p-10 text-center shadow-sm ${palette.panel}`}><div className={`mx-auto mb-6 flex h-14 w-14 items-center justify-center rounded-full border ${palette.icon}`}><img src="/A.png" alt="ArchiWiki" className="h-9 w-9 object-contain" /></div><p className="text-[11px] uppercase tracking-[0.22em] opacity-60">ArchiWiki</p><h1 className="mt-2 text-2xl sm:text-3xl font-bold tracking-tight">We'll be back shortly</h1><p className={`mx-auto mt-4 max-w-md text-sm leading-6 ${palette.muted}`}>ArchiWiki is temporarily unavailable while maintenance is being carried out. Please try again later.</p><div className={`mx-auto mt-7 h-px w-20 opacity-30 ${palette.accent}`}/></div></div>;
}
